...
**Config files — three in total:**

| File | Location | Owner |
|---|---|---|
| `errorprobe.yaml` | **Project root** (next to the binary) | User — the master config, hand-edited |
| `loki-config.yaml` | `~/.errorprobe/configs/loki-config.yaml` | Generated by errorprobe — do not edit |
| `vector.toml` | `~/.errorprobe/configs/vector.toml` | Generated by errorprobe — do not edit |
| `grafana/provisioning/datasources/loki.yaml` | `~/.errorprobe/configs/grafana/…` | Generated by errorprobe — do not edit |

`errorprobe.yaml` is the single source of truth. ErrorProbe reads it and renders the three downstream tool configs into `~/.errorprobe/configs/`. Generated files are overwritten on every `up` or `reload` — any manual edits to them will be lost.
